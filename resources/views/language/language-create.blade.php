...
<div class="container-fluid">
    ...
    <!-- Page Heading -->
    <h1 class="h3 mb-2 text-gray-800">{{clean( trans('instep-backend.create_language') , array('Attr.EnableID' => true))}}</h1>

    <!-- DataTales Example -->
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">{{clean( trans('instep-backend.create_language') , array('Attr.EnableID' => true))}}</h6>
        </div>
        <div class="card-body">
                        ...
                        <form action="{{route('language.store')}}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                <div class="col-xs-12 col-sm-12 col-md-12">
                                    ...
                                    <div class="form-group">
                                        <strong>{{clean( trans('instep-backend.code_lang') , array('Attr.EnableID' => true))}}</strong>
                                        <input type="text" name="code" class="form-control">
                                    </div>

                                    <div class="form-group">
                                        <strong>{{clean( trans('instep-backend.is_default') , array('Attr.EnableID' => true))}}</strong>
                                        
                                        <div class="form-check">
                                          <input class="form-check-input" type="radio" name="is_default" id="is_default1" value="1">
                                          <label class="form-check-label" for="is_default1"> {{clean( trans('instep-backend.yes') , array('Attr.EnableID' => true))}}  </label>
                                        </div>

                                        <div class="form-check">
                                          <input class="form-check-input" type="radio" name="is_default" id="is_default2" value="0">
                                          <label class="form-check-label" for="is_default2"> {{clean( trans('instep-backend.no') , array('Attr.EnableID' => true))}}  </label>
                                        </div>
                                    ...
                                    <div class="form-group">
                                        <strong>{{clean( trans('instep-backend.ltr_rtl') , array('Attr.EnableID' => true))}}</strong>
